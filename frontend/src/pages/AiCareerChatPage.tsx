...
function TypingIndicator() {
  return (
    <div className="flex items-end gap-3">
      <div className="h-8 w-8 rounded-full bg-[#2E4CFF] flex items-center justify-center shrink-0 shadow-md">
      ...
      <div className="chat-bubble-ai px-4 py-3">
        <div className="flex items-center gap-1.5">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="h-2 w-2 rounded-full bg-[#2E4CFF] animate-typing"
      ...
      <div className="max-w-4xl mx-auto flex flex-col" style={{ height: 'calc(100vh - 7rem)' }}>
        ...
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-800/60 shrink-0">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-[#2E4CFF] flex items-center justify-center shadow-lg shadow-[#2E4CFF]/20">
          ...
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-[#2E4CFF]/25 bg-[#2E4CFF]/10 text-[#2E4CFF] text-[11px] font-semibold">
          ...
          {isLoading ? (
            <div className="h-full flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                <div className="h-10 w-10 rounded-full border-2 border-[#2E4CFF] border-t-transparent animate-spin" />
                <p className="text-sm text-slate-500">Loading conversation...</p>
              </div>
            </div>
          ) : messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center py-10">
              <div className="h-16 w-16 rounded-2xl bg-[#2E4CFF]/10 border border-[#2E4CFF]/20 flex items-center justify-center mb-5">
                <Sparkles className="h-8 w-8 text-[#2E4CFF]" />
              </div>
              ...
              <div className="flex flex-wrap gap-2 justify-center max-w-lg">
                {promptSuggestions.map((s) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl text-xs font-medium text-slate-300 bg-slate-800/60 border border-slate-700/60 hover:border-[#2E4CFF]/40 hover:text-[#2E4CFF] hover:bg-[#2E4CFF]/10 transition-all"
                  >
                    {s}
                  </button>
                ))}
        ...
        <div className="shrink-0">
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto scrollable pb-1">
              {promptSuggestions.slice(0, 3).map((s) => (
                <button
                  key={s}
                  onClick={() => handleSend(s)}
                  className="shrink-0 px-3 py-1.5 rounded-xl text-xs font-medium text-slate-400 bg-slate-900/60 border border-slate-800/60 hover:border-[#2E4CFF]/40 hover:text-[#2E4CFF] transition-all"
                >
                  {s}
                </button>
              ))}
          ...
          <form onSubmit={handleFormSubmit} className="relative">
            <input
              ...
              disabled={isSending}
              className="w-full rounded-2xl border border-slate-800/80 bg-slate-900/60 backdrop-blur-md py-4 pl-5 pr-14 text-sm text-white placeholder-slate-600 focus:border-[#2E4CFF]/50 focus:outline-none focus:ring-2 focus:ring-[#2E4CFF]/20 transition-all shadow-xl disabled:opacity-70"
            />
            <button
              type="submit"
              disabled={isSending || !input.trim()}
              className="absolute right-2.5 top-2.5 h-9 w-9 rounded-xl bg-[#2E4CFF] hover:bg-[#1A32C7] flex items-center justify-center text-white shadow-md hover:shadow-[#2E4CFF]/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {isSending ? (
